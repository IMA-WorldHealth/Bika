  ...
  return {
    ...
    select: function (data) {
      var ordre = 'SELECT ', entities = '', colonnes = '', 
            tables = '', jconditions='', conditions = '', orderby = ' ORDER BY ', 
            limit = ' LIMIT ';
      ...
      for (item in data) {  // FIXME: Decompile into a map {xcond: fx(), ycond: fy()}
        if (item === 'entities') {
          ...
          data[item].forEach(function(o) {
            ...
            parts.push(escape_id(o.t) + '.' + escape_id(o.c) + dir);
          });
          orderby += parts.join(', ').trim();
        } else if (item === 'limit') {
          if (isInt(data[item])) limit += data[item];
        }
      }
      requette = ordre + colonnes + ' FROM ' + tables;
      requette += (jconditions || conditions) ? ' WHERE ' + jconditions + conditions : "" ;
      return requette.trim() + orderby + limit + ";";
    }
  };
}
