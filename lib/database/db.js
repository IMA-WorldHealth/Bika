...
function souder(chaines, motif, sep) {
  ...
  for(i; i < l; i++) { 
    chaine += motif + escape(chaines[i]) + sep;
  }
  return chaine;
}
...
function db(options) {
  ...
  return {
    ...
    update: function(table, data) {
      ...
      for (row_id in data.rows) {
        ...
        for(keyInfo in data.updateInfo) {
          ...
          colAndVal += escape(keyInfo) + affectation + value + separateur;
        }
      ...
      requette = statement + table + colAndVal + ' WHERE ' + row_id + affectation + propValue + ";";
    ...
    delete: function(table, ids) {
      ...
      function escapeNonInts(i) { return isInt(i) ? i : escape(i); }
      ...
      for (id in ids) { 
        if (ids[id] && ids.hasOwnProperty(id) && ids.propertyIsEnumerable(id)) {
          ...
          statement += id + joiner + in_block + ander;
        }
      }
    ...
    insert: function (table, rows) {
      ...
      rows.forEach(function (row) {
        ...
        for (key in row) {
          ...
          insert_value = (typeof row[key] === 'string') ? escape(row[key]) : row[key];
      ...
      for (item in data) {  // FIXME: Decompile into a map {xcond: fx(), ycond: fy()}
        if (item === 'entities') {
          ...
          data[item].forEach (function (jcond) {
            jconditions+= escape(jcond['ts'][0])+'.'+escape(jcond['c'][0])+' = '+ escape(jcond['ts'][1])+'.'+escape(jcond['c'][1]);
            if (jcond.l) { jconditions += ' ' + jcond.l + ' '; }
          });     
        } else if (item === 'cond') {
          data[item].forEach (function (cond) {
            if (cond.z === 'IN') {
              conditions += escape(cond.t) + '.' + escape(cond.cl) + ' ' + cond.z + ' ' + cond.v + '';
            } else {
              cond.v = (isInt(cond.v)) ? cond.v : escape(cond.v);
              conditions += escape(cond.t) + '.' + escape(cond.cl) + ' ' + cond.z + ' ' + cond.v + ' '; 
            }
            conditions += cond.l || "";
          });
